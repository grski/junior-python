...
The questions you'll encounter depend strictly on what you put in your CV. As an example, I'll give you here a list of questions I've encountered in job interviews. Additionally, I'll add a few of my own. I'll discuss all of them, at least in a few words.

## Django

1. Distinct in Django models

1. Django middleware - what is it, where is it used
   Answer: It's a piece of code that processes our request on input and output in some way. At this level, we handle, for example, authentication.

1. Django request path
   Answer: Middleware -> router -> views -> models/code/whatever -> middleware again (optional) -> client

1. Django select related

1. How migrations work in Django

1. Does makemigrations need a database connection?\
    Answer: No, it doesn't. Django compares models with the last saved state (probably hashes) and if it detects changes, it gets to work.

1. Does migrate need a database connection?\
    Answer: Yes - it introduces changes to the database, so absolutely.
...
1. Difference between Flask and Django\
    Answer: Google it yourself, or better yet, check it out yourself and compare, because it's simple.
...
1. Setting unique_together on a model attribute that has `null=True`. Does Django allow this? What's the risk?
   Answer: In the database, null == null is False, so (null, 1) and (null, 1) will be unique for the database. For Python too.

## Python

1. 3 favorite features of Python 3 versus Python 2
     Answer: The answers here are obviously very individual, so I won't give a template, but I, if I remember correctly, mentioned: f-strings, unicode as default encoding, walrus operator (assignment expressions).

1. What would you change in Python - give an example of a new PEP
     Answer: Very individual matter. I threw out something about removing the GIL. And speaking of GIL, it's time for the classic question of every Python dev interview.
...
1. Name the types of comprehensions in Python.
     Answer: This refers to, for example, List Comprehensions, Set Comprehensions, Dict Comprehensions

1. Give examples of the above, one for each type.
...
1. What are the differences between a tuple and a list?
     Answer: The main difference is that one can be mutated, the other cannot - a list can be changed after creation, a tuple cannot - after initialization, you can't change it. Besides that, there's a big difference in performance and implementation "under the hood." Because a tuple is immutable, the interpreter at the moment of initialization knows exactly how much space it will take, so it allocates only as much memory as it needs and not a byte more. A list, on the other hand, during initialization, is created in such a way that it always allocates more memory than needed for the actual user data, so that operations of adding something to the list work faster. If you create a 50-element list, under the hood it turns out that your list has allocated space not for 50 elements, but for, say, 90. You can find out the exact numbers yourself.

1. What are generators and how do they differ from lists?
       Answer: Generators are something that makes it easier for us to operate on large amounts of data because they're like "functions" that instead of loading a whole specified data set into memory, process values one by one and yield successive values. They differ from lists in that a list loads everything into memory, a generator doesn't, simplifying greatly.

1. Can you program functionally in Python?
     Answer: Yes. It's possible - Python is a language that allows programming in many paradigms such as functional, object-oriented, imperative, procedural. One could argue whether this programming would be "pure" functional (probably not) and so on, but it is possible in itself. Not a very popular option and not very promoted. Generally, some people look askance at this combination.

1. Encapsulation in Python - is it possible?
     Answer: Yes and no. On one hand, Python lacks access modifiers like private or public, but we have conventional designations that cause, for example, variables or functions starting with an underscore '_' to be rather private and shouldn't be used outside the given class. However, this is just a convention, not a language element; nothing will stop you from using a method with an underscore everywhere.
...
1. How does access to dict elements work?
      Answer: Based on the key, a hash is calculated using a specific algorithm. From this hash, a pointer offset to the reference is obtained.

1. Preserving the order of adding items to a dictionary in Python, how?
      Answer: In Python < 3.7: OrderedDict() from collections. In >= 3.7, dict() itself preserves order by default.

1. Differences between a list and a set.
      Answer: In a list, elements can repeat. In a set, they can't. On a set, we can perform certain operations that we can't on lists - just like on normal sets in mathematics - union, intersection, difference. In the simplest case, you can think of a set as a list without duplicates. It's also worth remembering that data in a set is unordered by design, so a set is completely unsuitable for ordering elements.

1. What type of objects do dict.keys(), dict.values(), dict.items() return?
      Answer: Dict.items() - obvious. A list of tuples. But not quite. Because if you look closer, it's a dict_items class, which isn't quite a list - it's kind of an extended class because it allows us to perform operations on it like on sets. Same with keys() and values(). So on objects returned by these functions, you can perform union, difference, or intersection operations from sets. TL;DR - these functions return an iterable set-like object.

1. Bitwise operators in Python. What do you know, what are they for?
      Answer:  << and >> , bitwise shift left and right
      & - bitwise conjunction, i.e., 'and'
      | - bitwise disjunction, i.e., 'or'
      ~ - bitwise negation, i.e., ~x = -x - 1
      ^ - bitwise exclusive or

1. When to use list comprehension and when a regular for loop?
      Answer: Where we can, we should use list comprehensions, as they're more readable and efficient in terms of execution time, because they're optimized by the interpreter and run at speed somewhere close to C speed, while regular for loops are executed normally by the interpreter at regular Python code speed. However, if we want to execute something with side effects or complex logic, sometimes it's better to stick with regular loops.
...
1. What's an iterator vs iterable? Difference between iterator and iterable.
...
1. Write a program that checks whether a string entered by the user is a palindrome (a string that reads the same forwards and backwards).

1. Write a program that counts how many times a given digit appears in a string of digits entered by the user.

1. Write a function that takes a list of numbers and returns their sum and arithmetic mean.

1. Write a program that creates a dictionary containing letter statistics (how many times each letter appears in the text).
...
Answer: Merge creates a 'merge commit,' while 'rebase' sort of pastes commits from the merged branch. Besides, using rebase, you can really mess up the git log. What's better? Depends on who you ask; there are different schools of thought.
1. Differences between Git and GitHub.
Answer: Git is a tool - a version control system, while GitHub is a service where we can host our Git repositories and generally collaborate with others.
...
1. What is REST?
   Answer: Google it
1. What's responsible for what types of content can be sent to/from the server?
   Answer: Headers, specifically content-type.
1. Where do parameters go in a request?
   Answer: In the URL. ? and &.
1. 403 vs 401
   Answer: Forbidden vs unauthorized. The difference is when a user is not logged in, meaning they don't have access, vs is logged in but doesn't have permission.
1. HTTP response codes 1xx 2xx 3xx 4xx 5xx
   Answer: Check wiki.
1. What is JWT?
   Answer: JWT is simply an authentication method where we have a specific three-part token generated using some secret key/certificate that contains a specific payload.
1. Is authentication possible already at the load balancer level so that the application doesn't have to do it additionally later?
   Answer: Absolutely. JWT ftw.
1. Request authentication methods: basic auth and token - describe and characterize how they differ.
   Answer: Google it
1. Where does the server get the username and password in basic auth?
   Answer: Header. Which one?
1. How does the browser know that a given response should be displayed as JSON and not, for example, plaintext?
   Answer: Again - CONTENT-TYPE
1. Difference between PUT and PATCH.
    Answer: One requires providing all serializer fields, the other doesn't. Why? Which method requires all fields, which method doesn't?
...
1. Let's say I type something in the browser window and click enter to go to a given page. What happens under the hood?
...
1. Write a function rock_paper_scissors() that simulates a random round of rock paper scissors and then displays which player, p1 or p2, won the round. Does someone always win? Can there be a tie? Try to make your solution as short as possible. Try not to use if statements at all. Yes, you can do it without them.
   I managed to write the function simulating a single random game in 5 lines of code total, without an if.

2. Now write code that calls such a simulation 10 times and prints its result to output.
...
2. What about division by 2 considering similar restrictions as above?
3. Write a function is_prime(number) that checks whether a given number is prime.
4. Same as above, but in one line of code.


## Statistics from Logs
This one will be in English. Task description and potential solution below. The whole thing can still be simplified and refactored, but I'm giving here the solution I scribbled together quickly during a live interview. 30 minutes.
...
Task shamelessly borrowed from an interview.

## Query Statistics

Create a class that will store the total execution time of a given query. As input, you'll get records that will contain the query ID and duration. Records can be partial, meaning one ID can have multiple records. Such values should be summed together.

Additionally, implement in this class a method `get_top_k_records` that will return a specified number of queries with the longest execution time.
...
Implement an Order Book that handles regular orders with a Price Limit (Limit Order).

Then add support for Iceberg Orders. What this is and how it works, you can read in technical documentation of exchanges. For example: SETSmm and Iceberg Orders, SERVICE & TECHNICAL DESCRIPTION from the London Stock Exchange. Or Google it.

We read from standard input successive orders and update the book in real-time. If a transaction occurs, we print it to standard output. After adding each order, we print the updated book to standard output.

### Input Data Format

JSON. Each order has a "type" field and an "order" field. Type contains the order type - Iceberg or Limit. Order is the order data.
...
Iceberg orders have an additional field - `peak`, which means the peak of that order.

Assume input data is valid.
...
When we read information about a new order and want to print the updated order book state, the JSON object we should print should have the following properties/attributes: `buyOrders` and `sellOrders`. Each order should contain fields "id", "price", and "quantity". Sorted by price. Buy orders non-ascending. Sell orders non-descending. If the price is the same, the time of order addition takes priority. Example:
...
I'm not including it here because it's too long.

## URL Shortener

In this task, please create a URL shortening service. Use Python and any framework. E.g., Django or FastAPI.

So a service that shortens URLs, e.g., from https://codesubmit.io/library/react makes http://short.est/GeAi9
...
- Endpoints:
  ...
  - /decode - decodes shortened to full.

- Both return JSON
- How encoding will be done is an implementation detail. Choose as you see fit. As long as you can encode and then decode. You don't need to attach a database - we can keep it in memory.

- Create documentation on how to run and use.
- Write tests.
- Use GIT and through commit history show your reasoning
...
What does this mean? Look in Google for `SSG` or `static site generator`. Generally, it's a piece of code, a program that generates static pages based on something, something opposite to the current approach, i.e., creating SPAs. In my case, I decided to create a very simple system for my own blog. Here I leave the implementation details to you. Let it be part of the task - write down requirements, list functionalities, etc. Let your imagination run wild. The minimum I expect is delivering functionality sufficient for running a blog - an index with a list of posts and individual posts.
...
There are comments in the code, so I won't describe it additionally. Another piece of code:
...
More broadly written on [my blog post](https://grski.pl/braindead.html), and the entire source code on GitHub - [braindead](https://github.com/grski/braindead). There's also the commit history, description, etc. Take a look.
